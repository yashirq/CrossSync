// CrossSync minimal client (ASCII-only): QR + auto-connect + file transfer
;(function(){
  ...
  var ui = {
    ...
    renderQR: function(roomId, data){
      ...
      qr.innerHTML = '<img src="'+data.qrCode+'" alt="QR Code" style="max-width:220px;">'
                   + '<p style="margin-top:10px;font-size:0.9rem;color:#64748b;">Scan with the other device</p>';
    },
  ...
  var App = { downloadItems: [],
    ...
    init: function(){
      ...
      this.socket.on('room-created', function(o){
        self.roomId=o.roomId; ui.setRoomId(o.roomId);
        try{ var u=new URL(location.href); u.searchParams.set('room',o.roomId); history.replaceState(null,'',u.toString()); }catch(e){}
        ui.setStatus('等待对端扫码加入...'); ui.setIndicator('connecting'); ui.showTransfer();
        fetchQR(o.roomId).then(function(d){ ui.renderQR(o.roomId,d); }).catch(function(){ ui.setStatus('QR generation failed'); });
      });
    ...
    createRoom: function(){ ui.setStatus('Creating room...'); this.socket.emit('create-room'); },
    joinRoom: function(roomId){ ui.setStatus('Joining room '+roomId+' ...'); this.socket.emit('join-room',{roomId:roomId}); },

    // auto-connect helpers
    shouldInitiate: function(peerId){ try{ return String(this.socket.id) < String(peerId); }catch(e){ return true; } },
    autoConnectToPeers: function(devices){ var self=this; var ids=(devices||[]).map(function(d){return d.id;}).filter(function(id){ return id && self.socket && id!==self.socket.id; }); if(ids.length===0) return; ids.forEach(function(id){ var dc=self.dataChannels.get(id); var ok= dc && dc.readyState==='open'; if(!ok && self.shouldInitiate(id)){ ui.setStatus('正在连接...'); ui.setIndicator('connecting'); self.connectToDevice(id); } }); },
    ...
    setupDataChannel: function(dc, peerId){ var self=this; dc._busy=false; dc.onopen=function(){ ui.setStatus('已连接，可开始传输'); ui.setIndicator('connected'); ui.showTransfer(); ui.setDeviceOnline(); ui.collapseConnection(); self.updateConnectedLabel(); }; dc.onclose=function(){ dc._busy=false; ui.setIndicator('offline'); self.updateConnectedLabel(); }; dc.onerror=function(){ dc._busy=false; ui.setIndicator('offline'); }; dc.onmessage=function(ev){ self.onDataMessage(ev,peerId); }; },

    // File transfer
    processFiles: function(fileList){ var files = Array.prototype.slice.call(fileList||[]); if(!files.length) return; var dc=null, targetId=null; var self=this; this.dataChannels.forEach(function(v,k){ if(!dc && v && v.readyState==='open'){ dc=v; targetId=k; } }); if(!dc){ alert('No peer connected yet'); return; } files.forEach(function(f,i){ setTimeout(function(){ self.sendFile(dc,targetId,f); }, i*100); }); },
    sendFile: function(dc, peerId, file){ if(dc._busy){ var self=this; return void setTimeout(function(){ self.sendFile(dc,peerId,file); },500); } dc._busy=true; var self=this; var chunkSize=64*1024, offset=0, index=0; var fileId=peerId+'_'+Date.now()+'_'+Math.random().toString(36).slice(2); dc.send(JSON.stringify({type:'file-info', fileId:fileId, name:file.name, size:file.size, totalChunks:Math.ceil(file.size/chunkSize)})); var reader=new FileReader(); var update=function(){ self.showFileProgress(file.name, Math.round((offset/file.size)*100)); }; reader.onload=function(){ var buf=reader.result; dc.send(JSON.stringify({type:'file-chunk-header', fileId:fileId, chunkIndex:index, chunkSize:buf.byteLength})); dc.send(buf); offset += buf.byteLength; index++; update(); if(offset<file.size){ if(dc.bufferedAmount>512*1024) setTimeout(readNext,50); else readNext(); } else { dc.send(JSON.stringify({type:'file-end', fileId:fileId})); dc._busy=false; self.showFileProgress(file.name,100); } }; reader.onerror=function(){ dc._busy=false; alert('Read file failed'); }; var readNext=function(){ var slice=file.slice(offset, Math.min(offset+chunkSize,file.size)); reader.readAsArrayBuffer(slice); }; update(); readNext(); },
  ...
  // ZIP pack and Clear actions
  App.zipAndDownloadAll = function(){
    if(!window.JSZip){ alert('JSZip 未加载'); return; }
    var items = (this.downloadItems||[]).slice();
    if(items.length===0){ alert('暂无可打包的文件'); return; }
    var zip = new JSZip();
    var addOne = function(it){ return fetch(it.url).then(function(r){ return r.blob(); }).then(function(b){ zip.file(it.name, b); }); };
    var seq = Promise.resolve();
    items.forEach(function(it){ seq = seq.then(function(){ return addOne(it); }); });
    seq.then(function(){ return zip.generateAsync({type:'blob'}); }).then(function(blob){ var url=URL.createObjectURL(blob); var a=document.createElement('a'); a.href=url; var ts=new Date().toISOString().replace(/[:.]/g,'-'); a.download='CrossSync-'+ts+'.zip'; document.body.appendChild(a); a.click(); setTimeout(function(){ document.body.removeChild(a); URL.revokeObjectURL(url); },500); }).catch(function(e){ console.error('zip error',e); alert('打包失败'); });
  };

  App.clearReceived = function(){
    try{ (this.downloadItems||[]).forEach(function(it){ if(it && it.url && typeof it.url==='string' && it.url.indexOf('blob:')===0){ try{ URL.revokeObjectURL(it.url); }catch(e){} } }); }catch(e){}
    var list=$('filesList'); if(list){ while(list.firstChild){ list.removeChild(list.firstChild); } }
    this.downloadItems = [];
    var panel=$('receivedFiles'); if(panel) panel.style.display='none';
  };
